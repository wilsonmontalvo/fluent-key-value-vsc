...
class ExpressionAnalyzer {
	...
	analyze(snipShortcut) {
		...
		this.resolver.setNodesDepth(expTree, null, 0);
	...
	evaluateExpression(node, contextOp) {
		...
		if (node.isLeafe()) {
			return this.resolver.resolve(null, null, node.root, node, node.parent)
		}
		else  
		if (!node.left) {
			if (operator == '(')
				...
				return this.resolver.resolve(null, node.root, node.right.root, node, node.parent);
		}
		else if (node.left.isLeafe() && node.right.isLeafe()) {
			return this.resolver.resolve(node.left.root, operator, node.right.root, node, node.parent);
		}
		else {
			let left = '';
			let right = '';
			...
			var result = this.resolver.resolve(left, operator, right, node, node.parent);
